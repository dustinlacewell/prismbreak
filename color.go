...
func rgb_lerp(top, bot RGB, amt float64) RGB {
	topR, topG, topB := colorToFloats(top)
	botR, botG, botB := colorToFloats(bot)

	r := uint8(lerp(topR, botR, amt) * 255)
	g := uint8(lerp(topG, botG, amt) * 255)
	b := uint8(lerp(topB, botB, amt) * 255)
...
func (c RGB) Lerp(o RGB, amt float64) RGB {
	mr, mg, mb := colorToFloats(c)
	or, og, ob := colorToFloats(o)

	c.R = uint8(lerp(mr, or, amt))
	c.G = uint8(lerp(mg, og, amt))
	c.B = uint8(lerp(mb, ob, amt))
